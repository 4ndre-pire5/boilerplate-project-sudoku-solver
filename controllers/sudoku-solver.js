...
class SudokuSolver {
  ...
  // ✅ Verifica se o valor pode ser colocado na coluna
  checkColPlacement(puzzleString, row, column, value) {
    const colIndex = column - 1;
    for (let i = 0; i < 9; i++) {
      if (puzzleString[i * 9 + colIndex] === value) return false;
    }
    return true;
  }

  // ✅ Verifica se o valor pode ser colocado na região 3x3
  checkRegionPlacement(puzzleString, row, column, value) {
    const rows = 'ABCDEFGHI';
    const rowIndex = rows.indexOf(row.toUpperCase());
    const colIndex = column - 1;
  ...
  // Função de resolução de Sudoku (mantida)
  solve(puzzleString) {
    const validation = this.validate(puzzleString);
    if (validation.error) return validation;
    ...
    const solveRecursive = (index = 0) => {
      ...
      for (let num = 1; num <= 9; num++) {
        ...
        if (
          this.checkRowPlacement(current, row, col, val) &&
          this.checkColPlacement(current, row, col, val) &&
          this.checkRegionPlacement(current, row, col, val)
        ) {
          puzzle[index] = val;
          if (solveRecursive(index + 1)) return true;
          puzzle[index] = '.';
